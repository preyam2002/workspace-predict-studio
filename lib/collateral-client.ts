...
export class CollateralClient {
  ...
  buildOpenPositionTx(ids: CollateralMarketIds, shareCoinId: string, floorValue: number): Transaction {
    const tx = new Transaction();
    const position = tx.moveCall({
      target: `${this.pkg}::studio_collateral::open_position`,
      arguments: [tx.object(ids.marketId), tx.object(shareCoinId), tx.pure.u64(floorValue)],
    });
