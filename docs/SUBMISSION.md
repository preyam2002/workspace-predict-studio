...
6. Show vault NAV vs secondary-market panel, PT/YT split, creator leaderboard, Walrus share link, and backtest panel.
7. Close with mainnet readiness: config shim is ready; Predict is testnet-only today; Margin composition stays roadmap until a verified Margin-to-Predict target exists.
...
- Local address inventory confirms the active wallet already holds `1999.999999` `STUDIO_LP` plus `0.250509634` SUI; other local aliases hold another `0.2331732` SUI. No local address has funded Predict dUSDC, funded DEEP, or funded dBUSDC.
- Secondary market is implemented with Cetus/mock fallback. DeepBook Spot pool creation is wired and uses official SDK testnet registry/package defaults, but current wallets have `0` funded DEEP and permissionless pool creation requires 500 DEEP.
- The live DEEP/SUI testnet pool did not unblock funding: 10 DEEP quoted at about `0.2702` SUI and 50+ DEEP returned no liquidity.
